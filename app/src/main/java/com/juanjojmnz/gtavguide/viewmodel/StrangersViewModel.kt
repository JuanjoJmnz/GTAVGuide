...
@HiltViewModel
class StrangersViewModel @Inject constructor(
    ...
    private fun loadMissions(filter: CharacterFilter) {
        viewModelScope.launch {
            ...
            val flow = when (filter) {
                CharacterFilter.ALL -> repository.getMissionsByType("STRANGERS_FREAKS")
                else -> repository.getMissionsByCharacterAndType(
                    filter.name, "STRANGERS_FREAKS"
                )
            }
            flow.collect { missions ->
                _uiState.update { it.copy(missions = missions, isLoading = false) }
            }
        }
    }
